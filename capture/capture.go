// Package capture 把数据包源按 TCP 流重组,经 GCP 分帧、密钥提取、0x4013 解密后产出应用层消息。
//
// 本包只做纯 Go 的离线回放(pcap 文件)与处理循环;实时抓包源(afpacket 是 cgo)由使用方
// 自行构造 gopacket.PacketSource 后交给 Engine.Process,这样 pcapdump 等分析工具无需 cgo。
package capture
...
// Process 是实时抓包/离线回放共用的处理循环:消费 src 直到耗尽,收尾时冲刷汇编器。
// 不关闭 Out(实时源可能重启);离线回放见 RunOffline / RunOfflineFiles。
